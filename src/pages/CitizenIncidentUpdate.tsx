...
export function CitizenIncidentUpdate() {
  ...
  const incidentLookupCategories = useMemo(
    () =>
      (lookupCategoriesData?.data || []).filter(
        (cat: any) => cat.add_to_incident_form,
      ),
            ...
            <Card className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {incidentLookupCategories
                  .filter((category: any) => category.code === "PRIORITY")
                  .map((category: any) => (
                    <DynamicLookupField
                      key={category.id}
                      category={category}
