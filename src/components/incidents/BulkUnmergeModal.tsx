...
export const BulkUnmergeModal: React.FC<BulkUnmergeModalProps> = ({
  ...
  const handleBulkUnmerge = async () => {
    ...
    try {
      ...
      if (data && data.failures && data.failures.length > 0) {
        setError(
          `${data.unmerged_count} unmerged, ${data.failures.length} failed`,
        );
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-[hsl(var(--foreground)/0.6)] backdrop-blur-sm items-center justify-center bg-opacity-50">
