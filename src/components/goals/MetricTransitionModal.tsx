...
import { useExecuteMetricTransition } from "../../hooks/useGoals";

interface MetricTransitionModalProps {
  metricId: string;
  metricName: string;
...
/**
 * Mirrors EvidenceTransitionModal but targets the metric workflow endpoints.
 * Same layout, same colour conventions — only the underlying mutation hook
 * differs.
 */
export const MetricTransitionModal: React.FC<MetricTransitionModalProps> = ({
  metricId,
  metricName,
  transitions,
  isOpen,
  ...
  const executeTransition = useExecuteMetricTransition();
  ...
  const handleSubmit = () => {
    ...
    const data: MetricTransitionRequest = {
          ...
          {executableTransitions.length === 0 ? (
            ...
            <>
              ...
              {selectedTransition && (
                <div>
                  <label
                    htmlFor="metric-transition-comment"
...
export default MetricTransitionModal;
