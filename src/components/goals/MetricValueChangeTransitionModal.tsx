...
import { useExecuteValueChangeTransition } from "../../hooks/useGoals";

interface MetricValueChangeTransitionModalProps {
  changeId: string;
  metricName: string;
...
/**
 * Approval modal for a single MetricValueChange. Shape mirrors
 * EvidenceTransitionModal so reviewers don't have to learn a new UI.
 */
export const MetricValueChangeTransitionModal: React.FC<
  MetricValueChangeTransitionModalProps
> = ({
  changeId,
  metricName,
  proposedValue,
  ...
  const executeTransition = useExecuteValueChangeTransition();

  const selectedTransition = useMemo(
    () =>
      transitions.find((tr) => tr.transition.id === selectedTransitionId) ??
      null,
  ...
  const handleSubmit = () => {
    ...
    const data: MetricValueChangeTransitionRequest = {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md mx-4">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700/60">
          <div>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
              {t("goals.components.metric.transitionModal.title")}
            </h3>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-700/30 text-sm">
            ...
            <>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">
                  {t("goals.components.metric.transitionModal.transition")}
                </label>
                <div
              ...
              {selectedTransition && (
                <div>
                  <label
                    htmlFor="value-change-transition-comment"
                    className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1"
                  >
                    {t(
                      "goals.components.metric.transitionModal.commentLabel",
                    )}{" "}
                    {commentRequired ? (
                  ...
                  <textarea
                    id="value-change-transition-comment"
                    value={comment}
                    ...
                    placeholder={t(
                      "goals.components.metric.transitionModal.commentPlaceholder",
                    )}
                  />
                </div>
              )}
            ...
            {executableTransitions.length > 0 && (
              ...
              >
                ...
                {executeTransition.isPending
                  ...
                  : t("goals.components.metric.transitionModal.submit")}
              </button>
            )}
          </div>
        </div>
...
export default MetricValueChangeTransitionModal;
